...
def test_resource_center_upload_list_download_and_unreference(client, test_user):
    ...
    assert upload_attraction.json()["validation"]["checksum"] == attraction_asset["sha256_checksum"]
...
def test_resource_center_accepts_document_uploads(client, test_user):
    csrf = login(client, test_user)
    project_id, _, attraction_id = create_project_dataset_attraction(client, csrf, suffix="documents")
    itinerary_id = create_itinerary(client, csrf, project_id=project_id, suffix="documents")

    csv_payload = b"name,city\nExample,Austin\n"
    upload_attraction = client.post(
        f"/api/projects/{project_id}/resources/attractions/{attraction_id}/assets",
        headers={"X-CSRF-Token": csrf},
        files={"file": ("catalog.csv", csv_payload, "text/csv")},
    )
    assert upload_attraction.status_code == 201
    attraction_asset = upload_attraction.json()["asset"]
    assert attraction_asset["preview_kind"] == "document"
    assert attraction_asset["detected_mime_type"] == "text/csv"

    csrf = login(client, test_user)

    attraction_download = client.get(f"/api/projects/{project_id}/resources/assets/{attraction_asset['id']}/download")
    assert attraction_download.status_code == 200
    assert attraction_download.content == csv_payload
    assert attraction_download.headers["content-disposition"].startswith("attachment")
    ...
    assert itinerary_asset["detected_mime_type"] == "application/pdf"
...
def test_resource_center_sanitizes_download_filename(client, test_user):
    csrf = login(client, test_user)
    project_id, _, attraction_id = create_project_dataset_attraction(client, csrf, suffix="download-name")

    png_payload = b"\x89PNG\r\n\x1a\n" + b"header-name"
    upload = client.post(
        f"/api/projects/{project_id}/resources/attractions/{attraction_id}/assets",
        headers={"X-CSRF-Token": csrf},
        files={"file": ('..\\unsafe/"bad\r\nname".png', png_payload, "image/png")},
    )
    assert upload.status_code == 201
    asset_id = upload.json()["asset"]["id"]

    download = client.get(f"/api/projects/{project_id}/resources/assets/{asset_id}/download")
    assert download.status_code == 200
    assert download.headers["content-disposition"] == 'inline; filename="badname.png"'
...
def test_resource_center_rejects_signature_extension_mismatch(client, test_user):
    ...
    jpeg_payload = b"\xff\xd8\xff" + b"bad-mismatch"
    upload = client.post(
        f"/api/projects/{project_id}/resources/attractions/{attraction_id}/assets",
        headers={"X-CSRF-Token": csrf},
        files={"file": ("looks-like-png.png", jpeg_payload, "image/png")},
    )
...
def test_resource_center_cleanup_deletes_only_eligible_unreferenced_assets(client, db, test_user):
    ...
    upload_unreferenced = client.post(
        f"/api/projects/{project_id}/resources/itineraries/{itinerary_id}/assets",
        headers={"X-CSRF-Token": csrf},
        files={"file": ("cleanup.jpg", b"\xff\xd8\xffcleanup", "image/jpeg")},
    )
...
def test_resource_center_cleanup_marks_indirectly_orphaned_itinerary_assets(client, db, test_user):
    ...
    upload_itinerary = client.post(
        f"/api/projects/{project_id}/resources/itineraries/{itinerary_id}/assets",
        headers={"X-CSRF-Token": csrf},
        files={"file": ("orphaned.jpg", b"\xff\xd8\xfforphaned", "image/jpeg")},
    )
    assert upload_itinerary.status_code == 201
